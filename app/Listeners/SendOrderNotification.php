...
class SendOrderNotification implements ShouldQueue
{
    ...
    {
        try {
            $event->model->notify(new OrderCreatedNotification(
                $event->model,
                $event->model->customer_phone
            ));

            Mail::to($event->model->customer_email)
                ->send(new OrderConfirmationMail($event->model));

            $adminMessage = "📦 *Order Baru Masuk*\n\n";
            $adminMessage .= "Invoice: {$event->model->invoice_id}\n";
            $adminMessage .= "Customer: {$event->model->customer_name}\n";
            $adminMessage .= "Email: {$event->model->customer_email}\n";
            ...
            Log::info('Order notifications sent (WhatsApp + Email)', [
